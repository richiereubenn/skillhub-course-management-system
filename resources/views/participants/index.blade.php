    ...
    <div x-data="{
        ...
        <div x-show="modalOpen" style="display:none" class="fixed inset-0 flex items-center justify-center z-50"
            ...
            <div class="bg-white rounded-lg p-6 z-10 w-full max-w-md border-2 border-blue-600">
                <h3 class="text-xl font-bold mb-4 text-gray-900" x-text="isEdit ? 'Edit Participant' : 'Add Participant'">
                </h3>
                ...
                <form method="POST" :action="isEdit ? '/participants/' + form.id : '{{ route('participants.store') }}'">
                    ...
                    <div class="space-y-3">
                        ...
                        <input x-model="form.email" name="email" placeholder="Email"
                            class="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-blue-600 focus:outline-none" />

                        <input x-model="form.phone" name="phone" placeholder="Phone"
                            class="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-blue-600 focus:outline-none" />

                        <input x-model="form.address" name="address" placeholder="Address"
                            class="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-blue-600 focus:outline-none" />
                    </div>
                                ...
                                <td class="py-3 px-4">
                                    <div class="flex gap-2">
                                        ...
                                        <form method="POST" action="{{ route('participants.destroy', $p) }}"
                                            class="inline-block" onsubmit="return confirm('Delete participant?')">
                                            @csrf @method('DELETE')
                                            <button
